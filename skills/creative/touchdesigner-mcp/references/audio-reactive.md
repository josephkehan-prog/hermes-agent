...
## Audio-Reactive GLSL (Proven Recipe, tested April 2026)

Alternative signal chain: feeds the full spectrum directly into a GLSL TOP as a texture, instead of per-band CHOP extraction.
...
3. **DO NOT use Lag CHOP or Filter CHOP for spectrum smoothing** — both operate in timeslice mode and expand 256 samples to 2400+, averaging all values to near-zero (~1e-06). This was the #1 audio sync failure in testing.
4. **Smoothing belongs in the GLSL shader** if needed, via temporal lerp with a feedback texture: `mix(prevValue, newValue, 0.3)`. This gives frame-perfect sync with zero pipeline latency.
5. **CHOP to TOP dataformat = 'r'**, layout = 'rowscropped'. Spectrum output is 256x2 (stereo). Sample at y=0.25 for first channel.
6. **Math gain = 10** (not 5). Raw spectrum values are ~0.19 in bass range. Gain of 10 gives usable ~5.0 for the shader.
7. **No Resample CHOP needed.** Control output size via AudioSpectrum's `outlength` param directly.
