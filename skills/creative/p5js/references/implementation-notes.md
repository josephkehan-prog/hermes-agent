...
Full code for the gotchas and patterns every production p5.js sketch needs.
Read this when implementing a sketch, not just when something breaks —
these patterns (seeded randomness, HSB, disabling FES) should be in from the
first line of code, not bolted on after.
