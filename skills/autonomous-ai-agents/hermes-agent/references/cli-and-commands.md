...
Full command surface for Hermes Agent: every `hermes` subcommand and every
in-session slash command. Read this when you need the exact flags for a
command, not just that the command exists.

For the authoritative, always-current list run `hermes --help` (and
`hermes <command> --help`) or `/help` in-session — this file can drift.
...
Credentials (OAuth + API keys, with pooling) are managed under `hermes auth` — see Credentials & Pools below.
...
(`hermes mcp install <name>`): see `references/native-mcp.md`.
...
patterns: see `references/webhooks.md`.
