...
| Path | `skills/osint-investigation` |
| Version | `0.1.0` |
...
Each source has a stdlib-only fetch script in `SKILL_DIR/scripts/`, e.g.:
...
All 11 fetchers (USAspending, Senate LD, OFAC SDN, ICIJ Offshore, OpenCorporates,
CourtListener, Wayback, Wikipedia/Wikidata, GDELT, plus the two above) follow
the same `--out data/<file>.csv` convention and write normalized CSV with a
header row; re-run idempotently. Full invocation examples for every script,
private-individual coverage-gap behavior, and rate-limit/API-key notes: read
`references/fetch-commands.md` before running fetchers.
...
## Person-centric OSINT + evidence discipline

Three keyless, stdlib-only helper scripts in `SKILL_DIR/scripts/` extend this
skill to individual people and handles, plus a hard anti-hallucination gate:

- **`footprint.py <username> [--json]`** — checks a curated set of 18
  platforms for a username's presence (GitHub, Reddit, YouTube, Twitch,
  etc.). Use it as the always-works local baseline for username recon before
  reaching for `sherlock`/`maigret`. Distinguishes reliable 404-based results
  from soft-404 sites flagged `manual`.
- **`dorkpack.py --name|--username|--email|--domain ... [--engine google|bing|both] [--json]`**
  — pure string generator (no network calls) that emits ready-to-paste
  Google/Bing dork queries for a name, handle, email, or domain. Use it to
  plan searches before driving the browser tool.
- **`evidence_check.py --report FILE --evidence PATH... [--json]`** — the
  anti-hallucination gate. Extracts every concrete claim (URL, email,
  @handle, phone, proper name) from a drafted report and flags any that do
  not appear verbatim in the fetched evidence corpus (footprint.py JSON,
  saved page text, dork-hit dumps). Exit code 2 means unsupported claims.

**Run `evidence_check.py` before emitting any report that names people,
handles, emails, or URLs.** Every concrete claim must trace to fetched
evidence — the same load-bearing rule as above, applied to person-centric
findings. Gaps stay gaps: an unconfirmed lead is reported as "possible" or
"unverified," never silently upgraded to "confirmed" because it sounds more
useful. If `evidence_check.py` flags a token, either find it in a re-fetch
or cut it from the report.
