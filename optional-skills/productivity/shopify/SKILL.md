...
## Inventory, Metafields, Storefront API, Bulk Operations & Webhooks

Full curl/GraphQL examples for: inventory adjustment/set (`inventoryAdjustQuantities` / `inventorySetQuantities`), metafields/metaobjects read+write (`metafieldsSet`), the public Storefront GraphQL API (separate endpoint/token), bulk operations for large dumps (`bulkOperationRunQuery` + JSONL polling), and webhook subscriptions + HMAC verification: read `references/advanced-operations.md` when a task needs any of these beyond basic CRUD.
